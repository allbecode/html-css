...
// $ultimos_acessos = $pdo->query("SELECT username, acao, data_hora FROM logs_acesso ORDER BY data_hora DESC LIMIT 10")->fetchAll();
//-------------------------------------------------
// $data = $_GET['data'] ?? null;

// if ($data) {
//     $stmt = $pdo->prepare("SELECT username, ip, acao, data_hora FROM logs_acesso WHERE DATE(data_hora) = ? ORDER BY data_hora DESC LIMIT 50");
//     $stmt->execute([$data]);
// } else {
//     $stmt = $pdo->query("SELECT username, ip, acao, data_hora FROM logs_acesso ORDER BY data_hora DESC LIMIT 10");
// }
...
<body>
    ...
    <form method="GET">
        <label>Data:</label>
        <input type="date" name="data" value="<?php echo $_GET['data'] ?? date('Y-m-d'); ?>">
        ...
        <label>Ação:</label>
        <!-- <input type="text" name="acao" value="<?php echo $_GET['acao'] ?? ''; ?>"> -->
        <select name="acao" id="">
            <option value="">...</option>
